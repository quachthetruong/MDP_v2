...
class MinerBackTestBase:
    ...
    def get_input_per_timestamp(self, timestamp: datetime,code:str) -> Dict[str, Node]:
        if timestamp in self.inputs:
            logging.info(f"cached {timestamp} in self.inputs")
            return self.inputs[timestamp]
        tmp = self.get_inputs(timestamp,code)
    ...
    def process(self, timestamp: datetime,code:str) -> Dict[str, Node]:
        assert (isinstance(timestamp, datetime))
        inputNodes = self.get_input_per_timestamp(timestamp,code)
        return self.process_all_symbols(inputNodes=inputNodes, target_symbols=self.target_symbols, timestamp=timestamp)

    def process_and_save(self, timestamp: datetime,code:str) -> Dict[str, Node]:
        output = self.process(timestamp)
        return output

    @abstractmethod
    def process_per_symbol(self, inputs: Dict[str, Node], symbol: str, timestamp,code:str) -> Node:
        raise NotImplementedError("Should implement this.")

    @validate_stages
    def build_stage(self, schedule: str, start_date: datetime, timestamp: datetime,code:str, func: Callable[[datetime,str], Dict[str, Node]]) -> Stage:
        nodes = func(timestamp,code)
    ...
    def mimic_backrun(self, cron_str: str, start_date: datetime, end_date: datetime, step: int,code:str) -> List[Stage]:
        ...
        while timestamp := schedule.next():
            if timestamp < end_date:
                stage = self.build_stage(schedule=cron_str, start_date=start_date, timestamp=timestamp,code=code,
