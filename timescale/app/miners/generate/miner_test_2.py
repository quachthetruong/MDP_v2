...
class MinerTest2(MinerBackTestBase):
    """
    test miner 2
    """
    ...
    def get_inputs(self, timestamp) -> List[Node]:
        def get_inputs(timestamp: datetime, target_symbols: List[str], input_streams: Dict[str, DataStreamBase]) -> List[Node]:
            ...
            thuyet_minh_dnsx_ky_truoc_timestamp = datetime(
    ...
    def process_per_symbol(self, inputs: List[pd.DataFrame], symbol: str, timestamp):
        def process_per_symbol(symbol: str, timestamp: datetime, inputs: Dict[str, Node]):
            ...
            def diff_series(series1, series2):
                ...
                return result
            current_symbol_inputs, other_symbol_inputs = symbol_split(
                symbol=symbol, inputs=inputs)

            wifeed_thuyet_minh_hientai_record = current_symbol_inputs[
                'wifeed_bctc_thuyet_minh_hientai']
            wifeed_thuyet_minh_ckng_record = current_symbol_inputs['wifeed_bctc_thuyet_minh_ckng']
            calculate_nam_chung_khoan_record = current_symbol_inputs[
                'calculate_bctc_nam_chung_khoan_hientai']
            logging.info(
                f"symbol: {symbol} va timestamp: {timestamp} {len(wifeed_thuyet_minh_ckng_record)} va {len(calculate_nam_chung_khoan_record)}")
            if len(wifeed_thuyet_minh_hientai_record) == 0 or len(wifeed_thuyet_minh_ckng_record) == 0 or len(calculate_nam_chung_khoan_record) == 0:
                return
            wifeed_thuyet_minh_ckng_record = wifeed_thuyet_minh_ckng_record.add_suffix(
                '_prev')
            ...
            logging.info(
                f"wifeed_thuyet_minh_merged mid {len(wifeed_thuyet_minh_merged)}")

            wifeed_thuyet_minh_merged.insert(
                0, 'symbol_', wifeed_thuyet_minh_merged.pop('symbol_'))
            wifeed_thuyet_minh_merged.insert(
                1, 'indexed_timestamp_', wifeed_thuyet_minh_merged.pop('indexed_timestamp_'))
            for c in wifeed_thuyet_minh_merged.columns:
                if c.endswith("_prev"):
                    wifeed_thuyet_minh_merged = wifeed_thuyet_minh_merged.drop(
                        c, axis=1)
                if not c.endswith("_prev") and np.issubdtype(wifeed_thuyet_minh_merged[c].dtype, np.number):
                    wifeed_thuyet_minh_merged[c] = diff_series(
                        wifeed_thuyet_minh_merged[c], wifeed_thuyet_minh_merged[f'{c}_prev'])
            ...
            logging.info(
                f"wifeed_thuyet_minh_merged after {wifeed_thuyet_minh_merged.info()}")
