...
class MinerTest(MinerBackTestBase):
    """
    Test miner
    """
    ...
    def process_per_symbol(self, inputs: List[pd.DataFrame], symbol: str, timestamp):
        def process_per_symbol(symbol: str, timestamp: datetime, inputs: Dict[str, Node]):
            ...
            ketquakinhdoanh_trongnam_other = ketquakinhdoanh_trongnam_other.add_suffix(
                '_other')
            ...
            ketquakinhdoanh_trongnam_merged.insert(
                0, 'symbol_', ketquakinhdoanh_trongnam_merged.pop('symbol_'))
            ketquakinhdoanh_trongnam_merged.insert(
                1, 'other_symbol', ketquakinhdoanh_trongnam_merged.pop('symbol__other'))
            ketquakinhdoanh_trongnam_merged.insert(
                2, 'indexed_timestamp_', ketquakinhdoanh_trongnam_merged.pop('indexed_timestamp_'))

            for c in ketquakinhdoanh_trongnam_merged.columns:
                if c.endswith("_other"):
                    ketquakinhdoanh_trongnam_merged = ketquakinhdoanh_trongnam_merged.drop(
                        c, axis=1)
                if not c.endswith("_other") and np.issubdtype(ketquakinhdoanh_trongnam_merged[c].dtype, np.number):
                    ketquakinhdoanh_trongnam_merged[c] = divide_series(
                        ketquakinhdoanh_trongnam_merged[c], ketquakinhdoanh_trongnam_merged[f'{c}_other'])
